...
function renderFileManagementRules() {
  ...
  $("file-management-rules").innerHTML = fileManagement.draft.map((rule, index) => {
    ...
    const actionFields = rule.operation === "compress" ? `<div class="rule-options"><label>Profile <select data-rule-field="profileId">${profileOptions}</select></label><label class="checkbox-line"><input type="checkbox" data-rule-field="disposition"${rule.disposition ? " checked" : ""}> Keep source</label><label class="checkbox-line"><input type="checkbox" data-rule-field="inPlace"${rule.inPlace ? " checked" : ""}> Compress in place</label><label>Destination <input data-rule-field="destination" value="${escapeHtml(rule.destination)}" placeholder="compressed/"${rule.inPlace ? " disabled" : ""}></label></div>` : ["copy", "move"].includes(rule.operation) ? `<div class="rule-options"><label>Destination <input data-rule-field="destination" value="${escapeHtml(rule.destination)}" placeholder="raws/"></label><label class="checkbox-line"><input type="checkbox" data-rule-field="preserve"${rule.preserve ? " checked" : ""}> Keep source subfolders</label></div>` : "";
    return `<article class="file-rule-card" data-rule-index="${index}"><div class="rule-card-header"><span class="muted">Rule ${index + 1}</span><button class="icon" type="button" data-rule-remove aria-label="Remove rule">×</button></div><div class="rule-line">For <select data-rule-field="representation">${fileManagementOptionList(representationSelectors, rule.representation)}</select> representations of <select data-rule-field="asset">${fileManagementOptionList(assetSelectors, rule.asset)}</select> assets → <select data-rule-field="operation">${fileManagementOptionList(operationSelectors, rule.operation)}</select></div>${actionFields}</article>`;
...
function renderFileManagementProfiles() {
  $("file-management-profiles").innerHTML = fileManagement.profiles.map(profile => { const settings = profile.settings || {}; const pending = profile.codec === "av1" || profile.codec === "avif"; const detail = profile.codec === "jpeg-xl" ? `Quality ${settings.quality ?? "—"}${settings.effort ? ` · Effort ${settings.effort}` : ""}` : profile.codec.toUpperCase(); return `<article class="profile-card"><div><h3>${escapeHtml(profile.name)}</h3><p class="muted">${escapeHtml(detail)}${pending ? " · pending" : ""}</p></div><div>${profile.is_builtin ? `<span class="muted">Built-in${pending ? " · pending" : ""}</span>` : `<button class="secondary" type="button" data-profile-edit="${escapeHtml(profile.id)}">Edit</button>`}</div></article>`; }).join("");
...
function renderFileManagementPlan(data) {
  ...
  const deltaLabel = delta < 0 ? `Estimated net space freed <strong>${formatBytes(-delta)}</strong>` : `Estimated net space added <strong>${formatBytes(delta)}</strong>`;
  $("file-management-plan-summary").innerHTML = `<div class="plan-summary-grid">${cards.map(([label, value, bytes]) => { const amount = label === "COMPRESS" ? `${formatBytes(value?.source_bytes || 0)} → ${formatBytes(value?.estimated_output_bytes || 0)}` : formatBytes(value?.[bytes] || 0); return `<article class="plan-card"><strong>${label}</strong><span>${Number(value?.file_count || 0).toLocaleString()} ${Number(value?.file_count || 0) === 1 ? "file" : "files"}</span><small>${amount}</small><em class="${Number(value?.estimated_storage_delta_bytes || 0) < 0 ? "storage-free" : Number(value?.estimated_storage_delta_bytes || 0) > 0 ? "storage-add" : ""}">${Number(value?.estimated_storage_delta_bytes || 0) < 0 ? "Frees " : Number(value?.estimated_storage_delta_bytes || 0) > 0 ? "Adds " : ""}${formatBytes(Math.abs(Number(value?.estimated_storage_delta_bytes || 0)))}</em></article>`; }).join("")}</div><div class="plan-summary-callouts"><span>${deltaLabel}</span><span>Peak temporary space required <strong>${formatBytes(summary.peak_temporary_bytes || 0)}</strong></span><span>Available space <strong>${formatBytes(summary.available_space_bytes)}</strong></span><span>Logical assets with no remaining archive representation <strong>${Number(summary.assets_with_no_surviving_representation || 0).toLocaleString()}</strong></span></div>`;
