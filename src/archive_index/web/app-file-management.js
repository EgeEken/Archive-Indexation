  ...
  $("file-management-rules").querySelectorAll("[data-rule-index]").forEach(card => {
    ...
    card.querySelectorAll("[data-rule-field]").forEach(control => {
      const update = () => {
        ...
        if (control.dataset.ruleField === "operation" && ["copy", "move"].includes(value) && !["copy", "move"].includes(fileManagement.draft[index].operation)) fileManagement.draft[index].preserve = true;
        fileManagement.draft[index][control.dataset.ruleField] = value;
        markRulesDirty();
...
function executionDestructive(execution) { return executionRows(execution).some(row => ["move", "delete"].includes(row.operation) && row.status === "pending") || executionRows(execution).some(row => row.operation === "compress" && row.status === "pending" && row.source_disposition === "replace"); }
...
function estimatedChangeMarkup(value) { const amount = Number(value || 0); const sign = amount > 0 ? "+" : amount < 0 ? "−" : ""; const className = amount > 0 ? "net-increase" : amount < 0 ? "net-reduction" : "net-neutral"; return `<strong class="${className}">${sign}${formatBytes(Math.abs(amount))}</strong>`; }
function renderExecution(execution) {
  ...
  if (canStart) {
    ...
    const compressed = pending.filter(row => row.operation === "compress").length;
    const blockedCompression = rows.filter(row => row.operation === "compress" && row.status === "excluded").length;
    ...
    const countMarkup = [["Copy", copies], ["Compress", compressed], ["Move", moves], ["Delete", deletes], ["Blocked compression", blockedCompression]].filter(([, count]) => count).map(([label, count]) => `<span>${label} <strong>${count}</strong></span>`).join("");
    panel.innerHTML = `<section class="execution-review"><h3>Review execution</h3><p>This frozen review contains the exact operations that will be attempted.</p><div class="execution-counts">${countMarkup}</div><p class="muted">Bytes written ${formatBytes(execution.estimated_bytes_written || 0)} · Bytes moved ${formatBytes(movedBytes)} · Bytes permanently deleted ${formatBytes(execution.estimated_bytes_removed || 0)} · Estimated total change ${estimatedChangeMarkup(execution.estimated_storage_delta)}</p><p class="muted">Temporary space required ${formatBytes(execution.temporary_space_upper_bound_bytes || 0)}${targetFolders.length ? ` · Target folders ${escapeHtml(targetFolders.join(", "))}` : ""}</p>${blockedCompression ? `<p class="muted">${blockedCompression} compression operation${blockedCompression === 1 ? " is" : "s are"} blocked by the current codec capability policy.</p>` : ""}${destructive ? `<p class="execution-warning">Move, Delete, or source-replacing Compress operations will modify source files. Delete operations permanently remove the confirmed files.</p><label class="checkbox-line"><input type="checkbox" data-execution-ack><span>I understand that this execution will modify source files.</span></label>` : ""}<div class="form-actions"><button type="button" class="primary-action" data-execution-start ${destructive ? "disabled" : ""}>Execute safe operations</button><button type="button" class="secondary" data-execution-discard>Discard review</button></div></section>`;
...
$("file-management-button").addEventListener("click", async () => { try { await refreshFileManagementEditor(); const active = await api("/api/file-management/executions/active"); renderExecution(active.execution); $("file-management-dialog").querySelector(".planning-badge").textContent = "Copy · Compress · Move · Delete"; $("file-management-dialog").showModal(); if (active.execution && ["running", "cancelling"].includes(active.execution.status)) void pollFileManagementExecution(); } catch (error) { fileManagementStatus(`File Management failed: ${error.message}`, true); } });
