    ...
    const MAX_VIEWER_ZOOM = 40;
     const state = { workspace: query.get("workspace"), page: Number(query.get("page") || 1), pageSize: Number(localStorage.getItem("archive-index-page-size") || query.get("page_size") || 60), total: 0, items: [], viewerItems: [], viewMode: ["groups", "selection"].includes(query.get("view")) ? query.get("view") : "gallery", selectionFilter: query.get("selection") || "all", groupPage: 1, groupPageSize: 10, viewerIndex: -1, viewerZoom: 1, viewerPanX: 0, viewerPanY: 0, viewerSmooth: true, activeJobId: null, assetRequest: 0 };
    ...
    function syncUrl() {
      ...
      if ($("representatives-only").checked) params.set("representatives", "1");
      ...
      if (state.viewMode === "selection") { params.set("view", "selection"); if (state.selectionFilter !== "all") params.set("selection", state.selectionFilter); }
    ...
    function filterParams() {
      ...
      if (state.viewMode === "selection") {
        if (state.selectionFilter === "recommended") params.set("recommended", "1");
        if (["selected", "rejected", "undecided"].includes(state.selectionFilter)) params.set("decision", state.selectionFilter);
      }
    ...
    function issueMarkup(issues) { return (issues || []).map((issue) => `<span class="badge">${escapeHtml(labels[issue] || issue)}</span>`).join(""); }
     function scoreMarkup(score) { return score == null ? "" : `<span class="quality">quality ${Number(score).toFixed(2)}</span>`; }
     function selectionStateMarkup(item, controls = false) { const decision = item.user_decision || "undecided"; const label = decision === "undecided" ? (item.auto_recommended ? "Recommended" : "Undecided") : decision[0].toUpperCase() + decision.slice(1); const recommendation = item.auto_recommended && decision === "undecided" ? `<span class="recommendation-mark">Recommended</span>` : ""; const actions = controls ? `<div class="selection-actions"><button type="button" class="selection-button${decision === "selected" ? " active" : ""}" data-decision="selected" data-asset-id="${escapeHtml(item.asset_id)}">Selected</button><button type="button" class="selection-button${decision === "rejected" ? " active rejected" : ""}" data-decision="rejected" data-asset-id="${escapeHtml(item.asset_id)}">Rejected</button><button type="button" class="selection-button${decision === "undecided" ? " active" : ""}" data-decision="undecided" data-asset-id="${escapeHtml(item.asset_id)}">Clear</button></div>` : ""; return `<div class="selection-state ${decision !== "undecided" ? `manual-${decision}` : item.auto_recommended ? "machine-recommended" : "unreviewed"}"><span>${label}</span>${recommendation}${actions}</div>`; }
    ...
    async function loadWorkspace() {
      ...
      $("folder").value = selected;
       $("search").value = query.get("q") || ""; $("media-type").value = query.get("media_type") || ""; $("sort-by").value = query.get("sort_by") || "capture_time"; $("direction").value = query.get("direction") || "desc"; $("representatives-only").checked = query.get("representatives") === "1"; state.selectionFilter = ["all", "recommended", "selected", "rejected", "undecided"].includes(query.get("selection")) ? query.get("selection") : "all"; $("page-size").value = String(state.pageSize); if (!["60", "120", "180"].includes($("page-size").value)) { state.pageSize = 60; $("page-size").value = "60"; }
    ...
    async function loadAssets() {
      ...
      try {
        ...
        $("status").textContent = $("representatives-only").checked && !data.grouping_available ? "Strict groups have not been built for this workspace yet." : "";
    ...
    function renderDetails(asset) {
      ...
      const dimensions = first.width && first.height ? `${first.width} × ${first.height}` : "Unavailable";
      return `<div class="dialog-inner"><div class="dialog-header"><div><h2 id="details-title">${escapeHtml(first.filename || "Asset details")}</h2><div class="muted">${escapeHtml(formatCapture(asset.capture_time, asset.capture_time_kind) || "Capture time unavailable")}</div></div><button id="details-close" class="secondary" type="button">Close</button></div><section><h3>Overview</h3><dl class="kv"><dt>Dimensions</dt><dd>${escapeHtml(dimensions)}</dd><dt>File size</dt><dd>${escapeHtml(formatBytes(first.size_bytes))}</dd><dt>Capture time</dt><dd>${escapeHtml(formatCapture(asset.capture_time, asset.capture_time_kind) || "Unavailable")}</dd></dl></section>${renderTechnicalDetails(first)}<section class="section"><h3>Technical quality</h3>${asset.physical_files.map((file, index) => renderQuality(file, index > 0)).join("")}</section>${asset.physical_files.map(renderComponentProblems).join("")}</div>`;
    }
    function renderQuality(file, showFilename) { if (file.media_type === "video" || file.components?.quality?.status === "not_requested") return `<div class="quality-unsupported">Technical quality review is not supported for video yet.</div>`; const score = file.quality_score == null ? "Unavailable" : Number(file.quality_score).toFixed(2); const components = file.quality_components || {}; const metrics = [["Focus", "focus"], ["Exposure", "exposure"], ["Contrast", "contrast"], ["Noise", "noise"]]; return `<div class="file-card">${showFilename ? `<div class="muted">${escapeHtml(file.filename)}</div>` : ""}<div class="quality-summary"><strong>Overall technical quality</strong><span class="quality-score-box"><span class="quality-score">${score}</span></span></div>${metrics.map(([label, key]) => { const value = components[key]; const number = value == null ? null : Number(value); const negative = key === "noise"; const display = negative && number != null ? 1 - number : number; return `<div class="metric"><span>${label}</span><div class="bar${negative ? " negative" : ""}"><span style="--value:${display == null ? 0 : Math.max(0, Math.min(100, display * 100))}%"></span></div><span>${display == null ? "—" : display.toFixed(2)}</span></div>`; }).join("")}</div>`; }
    function renderTechnicalDetails(file) { const rows = [["Camera", cameraValue(file), ""], ["Lens", metadataValue(file, ["LensModel", "LensMake"]), ""], ["Aperture", metadataValue(file, ["FNumber", "ApertureValue"]), "Aperture"], ["Shutter speed", metadataValue(file, ["ExposureTime", "ShutterSpeedValue"]), "ExposureTime"], ["ISO", metadataValue(file, ["ISOSpeedRatings", "PhotographicSensitivity"]), "ISOSpeedRatings"], ["Focal length", metadataValue(file, ["FocalLength", "FocalLengthIn35mmFilm"]), "FocalLength"]]; const visible = rows.filter(([, value]) => value); return visible.length ? `<section class="section"><h3>Technical details</h3><dl class="kv">${visible.map(([label, value]) => `<dt>${label}</dt><dd>${escapeHtml(value)}</dd>`).join("")}</dl></section>` : ""; }
    ...
    function metadataValue(file, keys) { const exif = file.metadata?.exif || {}; for (const key of keys) { if (exif[key] != null && readable(exif[key])) return formatMetadataValue(key, exif[key]); } return ""; }
    ...
    async function startIndex() { try { await api("/api/index", { method: "POST" }); $("status").textContent = ""; await loadJobs(); } catch (error) { $("status").textContent = error.message; } }
     function setupFilters() { ["folder", "media-type", "sort-by", "direction", "representatives-only"].forEach((id) => $(id).addEventListener("change", () => { state.page = 1; syncUrl(); loadAssets(); })); $("page-size").addEventListener("change", () => { state.pageSize = Number($("page-size").value); localStorage.setItem("archive-index-page-size", state.pageSize); state.page = 1; syncUrl(); loadAssets(); }); let debounce; $("search").addEventListener("input", () => { clearTimeout(debounce); debounce = setTimeout(() => { state.page = 1; syncUrl(); loadAssets(); }, 250); }); $("selection-tools").querySelectorAll("[data-selection-filter]").forEach((button) => button.addEventListener("click", () => { state.selectionFilter = button.dataset.selectionFilter; state.page = 1; updateSelectionFilterButtons(); syncUrl(); loadAssets(); })); updateSelectionFilterButtons(); }
     function updateSelectionFilterButtons() { $("selection-tools").querySelectorAll("[data-selection-filter]").forEach((button) => { const active = button.dataset.selectionFilter === state.selectionFilter; button.classList.toggle("active", active); button.setAttribute("aria-selected", String(active)); }); }
     function setViewMode(mode, load = true) {
       state.viewMode = ["groups", "selection"].includes(mode) ? mode : "gallery";
       const groups = state.viewMode === "groups";
       const selection = state.viewMode === "selection";
       if (selection && !query.get("sort_by")) { $("sort-by").value = "quality"; $("direction").value = "desc"; }
       ["filters", "status", "pager-top", "gallery", "pager-bottom"].forEach((id) => $(id).classList.toggle("hidden", groups));
       $("selection-tools").classList.toggle("hidden", !selection);
       $("groups-view").classList.toggle("hidden", !groups);
       $("gallery-view-toggle").setAttribute("aria-selected", String(!groups && !selection));
       $("groups-view-toggle").setAttribute("aria-selected", String(groups));
       $("selection-view-toggle").setAttribute("aria-selected", String(selection));
       updateSelectionFilterButtons();
       syncUrl();
       if (load) (groups ? loadGroups() : loadAssets());
     }
     function renderGroup(group) {
       const members = group.members.map((item, index) => { const preview = item.thumbnail_url ? `<img class="group-thumb" loading="lazy" src="${item.thumbnail_url}" alt="${escapeHtml(item.filename)}" onerror="this.replaceWith(Object.assign(document.createElement('div'), {className:'group-thumb placeholder', textContent:'Preview unavailable'}))">` : `<div class="group-thumb placeholder">Preview unavailable</div>`; return `<div class="group-member${item.is_representative ? " representative" : ""}"><button class="group-photo" type="button" data-group-index="${index}" aria-label="View ${escapeHtml(item.filename)}">${preview}</button><div class="group-caption"><span title="${escapeHtml(item.filename)}">${escapeHtml(item.filename)}</span>${item.quality_score == null ? "" : `<span class="muted">quality ${Number(item.quality_score).toFixed(2)}</span>`}${item.is_representative ? `<span class="representative-mark">Representative</span>` : ""}${selectionStateMarkup(item, true)}</div></div>`; }).join("");
    ...
    $("problems-button").addEventListener("click", showProblems);
     $("gallery-view-toggle").addEventListener("click", () => setViewMode("gallery")); $("groups-view-toggle").addEventListener("click", () => setViewMode("groups")); $("selection-view-toggle").addEventListener("click", () => setViewMode("selection"));
    ...
    $("viewer-stage").addEventListener("wheel", (event) => {
      ...
      if (next === 1) { state.viewerPanX = 0; state.viewerPanY = 0; } else { state.viewerPanX = pointX - contentX * next; state.viewerPanY = pointY - contentY * next; }
