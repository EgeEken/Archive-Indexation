...
class VisualizationBenchmarkTests(unittest.TestCase):
    ...
    def test_visualization_lod_prep_handles_archive_scale(self):
        ...
        vector_start = source.index("function visualizationThumbnailSize")
...
for (const count of [10000,50000,100000]) {{
  ...
  const timelineView={{key:`bench-${{count}}`,timeMode:'capture',timelineCaches:new Map(),centerTime:timeline.at(-1).time/2,visibleSpan:timeline.at(-1).time,baseScale:1}};
  ...
  const vectorView={{key:`bench-${{count}}`,data:{{browser_revision:'1'}},lodCaches:new Map(),vectorDisplayCaches:new Map(),baseCellWorld:1,gridOriginX:0,gridOriginY:0,centerX:.5,centerY:.5,scale:400,baseScale:400}};
  ...
  started=performance.now(); const vectorDisplay=buildVectorDisplayCache(vectorView,vector,vectorCandidateLevel(vectorView,0),visualizationThumbnailSize(vectorView,'vector')); const vectorDisplayMs=performance.now()-started;
  const geo=Array.from({{length:count}},(_,i)=>({{asset_id:`g-${{i}}`,longitude:-180+(i%1000)*.36,latitude:-80+Math.floor(i/1000)*.8}}));
  const geoView={{key:`bench-${{count}}`,data:{{browser_revision:'1'}},lodCaches:new Map(),baseCellWorld:.1,centerX:.5,centerY:.5,scale:400,baseScale:400}};
  ...
  results.push({{count,timelineMs,vectorMs,vectorDisplayMs,vectorGroups:vectorDisplay.groups.length,geoMs,timelineOccupied:timelineCache.occupied.size,vectorCells:vectorCache.cells.size,geoCells:geoCache.cells.size,timelineVisible,vectorVisible,geoVisible}});
}}
console.log(JSON.stringify(results));'''
        ...
        for item in results:
            ...
            print(f"visualization-lod benchmark assets={item['count']} timeline={item['timelineMs'] / 1000:.3f}s vector={item['vectorMs'] / 1000:.3f}s vector-display={item['vectorDisplayMs'] / 1000:.3f}s geo={item['geoMs'] / 1000:.3f}s")
