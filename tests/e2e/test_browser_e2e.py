...
class BrowserE2ETests(unittest.TestCase):
    ...
    def test_file_management_and_representation_comparison_workflow(self) -> None:
        ...
        self.page.mouse.down()
        self.page.mouse.move(bounds["x"] + bounds["width"] * .02, handle_box["y"] + 8, steps=3)
        self.page.mouse.up()
        self.assertLessEqual(int(handle.get_attribute("aria-valuenow")), 3)
        handle_box = handle.bounding_box()
        self.page.mouse.move(handle_box["x"] + 1, handle_box["y"] + 8)
        self.page.mouse.down()
        self.page.mouse.move(bounds["x"] + bounds["width"] * .98, handle_box["y"] + 8, steps=3)
        self.page.mouse.up()
        self.assertGreaterEqual(int(handle.get_attribute("aria-valuenow")), 97)
