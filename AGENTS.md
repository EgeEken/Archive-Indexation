...
Phase 10A is complete. Logical assets remain distinct from their physical files and persisted representation relationships. Details exposes the preferred representation and available alternates with Open, Compare, and Explorer actions; comparison uses existing viewer/details paths and does not duplicate the Details UI. JPEG XL is decoded through the installed `imagecodecs`/libjxl runtime. Its source remains canonical, while indexing records decoder/preview provenance and may create only an app-owned display JPEG under `.archive-index/`; no source file is modified and no general-purpose proxy generation is introduced. AVIF encoding remains a capability/research result, not an executor.

Schema migration 24 adds compression profiles, ordered file-management rulesets and rules, presets, workspace active-ruleset state, and display-preview provenance. Profiles store codec/container/settings; rules match indexed media type, extension, folder, role, manual state, and size and describe planned compress/copy/move actions. The editor and APIs persist these records and produce a deterministic dry-run plan over online in-scope indexed files. Plans sort by relative path and ID, report missing profiles, unsupported executors, source overwrites, target collisions/existing targets, and require confirmation for every operation. Phase 10A has no file executor: dry runs never change source media or filesystem paths.

JXL display decoding is the only production codec addition in this phase. The read-only benchmark harness reports the installed JPEG XL decoder version and decode timings for existing fixtures; it does not compress sources. No map/network/account dependency, OCR, face processing, AV1 encoding, semantic clustering, or automatic delete path was added. Production compression jobs, resumability, output validation, source/derivative execution, and safe-to-delete reports remain deferred to the later executor phase.
