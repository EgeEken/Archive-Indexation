...
Vector Cloud uses the same world camera and a cached approximately 512×512 world-anchored Gaussian density texture keyed by filtered data revision, with sqrt normalization and no semantic axis names or fake clusters. The density raster and points use the same explicit world-to-screen transform; representative thumbnails are drawn at their own PCA coordinates, not cell centroids. Stable x/y/LOD cells control aggregation while smooth zoom-relative thumbnail sizing controls visual footprint independently. Image/video fallback colors remain restrained, grouped count badges refine on click, and selected assets use the normal viewer path. The opt-in synthetic projection benchmark covers 10k, 50k, and 100k 8-dimensional vectors. On the local test machine, `_fit_pca` measured 0.026s, 0.101s, and 0.200s respectively with the 10,000-asset fit cap. No separate browser paint benchmark was recorded; all three renderers remain Canvas-based with O(N) data refresh work and no per-asset DOM nodes. Geo and Vector occupy the available workspace height on desktop; Timeline uses the same Canvas surface with an adaptive bounded height. JPEG XL remains recognized by indexing and the supported-extension UI but is explicitly in the decoder-gap set: no libjxl, djxl, imagecodecs, Pillow plugin, or native decoder is bundled, so `.jxl` decode remains deferred to Phase 10. Groupings expose stable `Group N` labels for real multi-image groups. Focused backend, projection, frontend-contract, and three-test Playwright Chromium coverage were added. Broad grouping, responsive/mobile overhaul, and Phase 10 remain deferred.
