...
This phase is implemented. The accepted production interface remains a standard-library localhost server with vanilla HTML/CSS/JavaScript. Phase 10A planning and Phase 10B safe Copy/Move/Delete execution are implemented without changing the search/model architecture; production compression remains deferred. The following decisions describe the accepted browser shell and search behavior; later Phase 9.3A Configure/runtime changes are recorded below.
...
- Phase 10A validates an actual `libjxl` runtime through `imagecodecs`, app-owned JPEG XL display-preview provenance, external JXL representation handling, lazy vectorized Difference generation, and read-only RAW inspection. Production JXL/AVIF encoding remains deferred until Phase 10C; Phase 10B handles only confirmed Copy/Move/Delete.
...
The File Management dialog shows the supported mutation scope: Copy, Move, and Delete. Compression remains visibly blocked until Phase 10C.
...
Copy and Move rules use explicit left-aligned option wrappers. Their helper updates during destination input without replacing the input. Built-in selected RAW cleanup moves flat to `raws`; selected JPEG/PNG cleanup copies flat to `jpgs`; both default to Rename conflicts and do not recreate source folders. New rules persist `conflict_policy`; legacy `rename_on_conflict` values are read only as compatibility input. Dry-run target resolution uses case-insensitive Windows semantics and deterministic `file (1).ext`, `file (2).ext` suffixes across existing entries and earlier planned outputs. Source-replacing compression in place is represented as an intentional `replaces_source_in_place` operation; keep-source compression at the same target must rename, skip, or report a blocker. AV1 capability is probed once from the local FFmpeg `-version` and `-encoders` output. The current runtime exposes `libsvtav1`, `libaom-av1`, `librav1e`, and hardware AV1 entries, but the archival AV1 profile remains planning-blocked because production encoding settings and the executor are not implemented. Capability blockers are summarized once rather than duplicated per file. The comparison slider has an in-flow sizing layer so zoom 1 fits the complete image, and RAW loading text lives beside the centered EV value without changing image-stage geometry. No archive source mutation is performed by previews or planning.
...
Migration 26 persists `file_management_execution` and `file_management_execution_operation`. A run stores the plan digest, frozen summary/metadata, actual effects, refresh status, and cancellation state; each operation stores its deterministic position, phase, operation type, source identity/path/size/mtime/SHA-256, target, conflict policy, reviewed-target size/mtime/SHA/type snapshot, rule/profile snapshot, conflicts/blockers, stage, temp path, output hash/size, progress, attempts, and error. Supported execution is single-threaded in `Copy → Move → Delete` order. Compress rows are persisted as excluded with `Production compression execution is not enabled until Phase 10C.`
...
Copy, Move, and Compress rules use explicit `Rename`, `Skip`, or `Overwrite` conflict policies, defaulting to Rename. Rename targets are frozen deterministically; Skip creates a persisted skipped row without writing; Overwrite is allowed only for an exact reviewed regular-file target snapshot and uses authorized replacement, never a newly appearing target. Legacy `rename_on_conflict: false` maps to Skip and `true` maps to Rename; new saves persist only `conflict_policy`. Built-in selected RAW cleanup is a Move to flat `raws`, selected JPEG/PNG cleanup is a Copy to flat `jpgs`, and source-folder recreation is disabled by default. The UI uses a busy local review state, hides the digest, materializes one representative plan row at a time with bounded Show more batches, and reports `Written … · Removed … · Total change …` with signed net-change colors. Large-copy progress writes are throttled to time/byte intervals while final and cancellation updates remain persisted. Synthetic Phase 10B throughput checks use both a large file and photo-like batches; no real archive is used for mutation testing.

The explicit deletion invariant is: indexing, browsing, and maintenance never delete source media; cleanup is not automatic for rejected or redundant files; an exact source may be deleted only by an explicitly reviewed and confirmed File Management Delete operation after execution-time validation. Compression replacement deletion remains deferred until Phase 10C. Future AV1 planning profiles are `AV1 1080p60 Fast`, `AV1 1080p60 Very Fast`, `AV1 4K120 Fast`, and `AV1 4K120 Very Fast`; max-resolution/FPS caps never upscale, the current default candidate is `AV1 4K120 Very Fast`, and exact encoder mapping is deferred to Phase 10C.
