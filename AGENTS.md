...
Schema migration 24 adds compression profiles, ordered file-management rulesets and rules, presets, workspace active-ruleset state, and display-preview provenance. Migration 25 adds persistent File Management execution and operation snapshots. The deterministic dry-run summary separates candidate and executable operations; primary storage deltas, estimated net space freed, and `temporary_space_upper_bound_bytes` include only operations without conflicts or blockers. The browser must prepare an execution with the analyzed plan digest; the backend re-runs the planner, persists the complete immutable snapshot, and only then permits explicit confirmation and execution.
...
Migration 25 persists `file_management_execution` and `file_management_execution_operation`. A run stores the plan digest, frozen summary/metadata, actual effects, refresh status, and cancellation state; each operation stores its deterministic position, phase, operation type, source identity/path/size/mtime/SHA-256, target, rule/profile snapshot, conflicts/blockers, stage, temp path, output hash/size, progress, attempts, and error. Supported execution is single-threaded in `Copy → Move → Delete` order. Compress rows are persisted as excluded with `Production compression execution is not enabled until Phase 10C.`
