...
This phase is implemented. The accepted production interface remains a standard-library localhost server with vanilla HTML/CSS/JavaScript. Phase 10A planning, Phase 10B safe Copy/Move/Delete execution, and Phase 10C validated JPEG XL keep-source execution are implemented without changing the search/model architecture; source replacement, AV1, and AVIF remain capability-gated as documented below. The following decisions describe the accepted browser shell and search behavior; later Phase 9.3A Configure/runtime changes are recorded below.
...
- Phase 10A validates an actual `libjxl` runtime through `imagecodecs`, app-owned JPEG XL display-preview provenance, external JXL representation handling, lazy vectorized Difference generation, and read-only RAW inspection. Phase 10C adds production JPEG XL keep-source execution; source replacement, AV1, and AVIF remain blocked by the capability contracts below.
...
Vector Cloud uses the same world camera and a cached approximately 512×512 world-anchored Gaussian density texture keyed by filtered data revision, with sqrt normalization and no semantic axis names or fake clusters. The density raster is not rebuilt during pan or zoom; representative thumbnails are drawn at their own PCA coordinates, not cell centroids. Stable integer x/y/LOD cells are cached by data/filter/revision, visible cells are selected by world-range iteration, neighboring levels crossfade with smoothstep, and LOD derives from rendered thumbnail footprint rather than viewport size. Final display grouping is two-stage: a globally anchored candidate cell is selected about 0.75 thumbnail long-edge wide, then candidate rectangles are consolidated when their actual aspect-aware rectangles overlap at least 55% of the smaller rectangle. A screen-space hash with bounded neighboring buckets avoids O(N²); each group compares candidates only to its stable anchor, preventing transitive chains. Group counts sum candidate counts, representatives use highest finite quality then asset ID, and grouping is pan-invariant and cached by browser revision/filter signature plus scale/LOD band. Image/video fallback colors remain restrained, grouped count badges refine on click, and visualization points open the existing fullscreen viewer directly. Timeline grouped buckets pass sorted member IDs to that viewer, which requests additional asset details only when navigating. Viewer Info reuses the existing Details side panel; there is no visualization-specific selection panel. All visualization thumbnails use one natural-aspect long-edge rectangle for draw, border, hover, badge anchor, and exact reverse-painter-order hit testing. Render order is stable by finite quality ascending then asset ID, and hover adds only a final border/glow overlay. Geo local metric grids iterate both visible directions from world-zero anchored indices, with one-cell metric scale bars and a fixed fit/load reference latitude. Zoom LOD uses a 420ms absolute last-zoom idle window and a 240ms settle crossfade; panning never restarts or blurs that transition. The Timeline canvas fills the remaining visualization workspace and its lower area remains wheel/pan interactive. The opt-in synthetic benchmarks cover PCA and Timeline/Vector/Geo preparation at 10k, 50k, and 100k assets; the latest local run measured PCA 0.023/0.098/0.189s, Timeline 0.019/0.074/0.118s, Vector LOD 0.004/0.010/0.004s plus display grouping 0.013/0.003/0.004s, and Geo 0.006/0.012/0.029s. No browser paint benchmark was recorded; all three renderers remain Canvas-based with no per-asset DOM nodes. Geo uses a bundled 251,749-byte Natural Earth 1:110m reduction, fades to a local GPS-relative coordinate field with graticule, north indicator, and metric scale bar at deep zoom, and makes no map-network calls. Spatial views are contained in the desktop viewport through a flex containment chain; the filter sidebar scrolls internally and the Timeline fills the remaining canvas surface. Search input uses a 250ms latest-query debounce, immediate clear/Enter handling, generation-checked aborts, staged 275/500ms polling, and full-ranking cache reuse across allowed filters while preserving video MAX-frame semantics. RAF thumbnail invalidations are coalesced per active visualization. JPEG XL is recognized by indexing and supported-extension UI and is decoded through the installed `imagecodecs`/libjxl runtime; app-owned display previews retain explicit provenance and no map/network calls or source mutation are involved. Groupings expose stable `Group N` labels for real multi-image groups. Phase 9.4 remains complete; Broad grouping and responsive/mobile overhaul remain deferred, while Phase 10A planning and Phase 10B safe Copy/Move/Delete execution are complete; production compression remains deferred to Phase 10C.

## Phase 10A — representation inspection and safe planning

Phase 10A second pass is complete. File Management is a first-class workspace-header action with structured Rules, Compression profiles, and Plan sections. Built-in Archive cleanup and Keep selected only presets are immutable and synchronized from code-owned definitions on every workspace refresh; edits become Custom Ruleset, while custom presets can be saved, renamed, and deleted. The normal UI contains no rules/settings JSON or raw plan output. Rules use plain-language semantic asset and representation selectors, the planner evaluates every matching rule, supports planned Delete, reports destination collisions with filenames and paths, and exposes a review step before execution. Conventional archive rules match only JPEG/PNG; JPEG XL, AVIF, and WebP are compressed-image representations. JXL profiles expose quality 80/60/40 with effort 7; native distance remains derived storage detail.

Logical assets remain distinct from their physical files and persisted representation relationships. Details exposes compact SVG eye/Explorer actions; the preferred eye uses the normal fullscreen viewer, while a non-preferred rendered representation compares only with the current preferred representation. The comparison overlay defaults to Slider, keeps the reference on the left and compressed representation on the right, and offers side-by-side mode without a target dropdown or standalone compressed preview. Both images always occupy their full frame; slider movement clips the reference layer and does not change image geometry. The shared camera uses explicit fullscreen, pane, or slider frames, synchronizes zoom/pan, clamps once against all active frames, and uses pixelated rendering when magnified. Pair data is keyed by both physical IDs and disposed with the camera on close. Comparison previews and metrics use the actual decoded physical pair; normal UI reports rounded `MSE N` with a continuous green/yellow/red scale, while backend PSNR remains diagnostic. Max-pixel MSE is Difference-internal and its inverse-log scale is shown only in the Difference legend. MSE is the true per-channel pixel mean and zero means pixel-identical. The deterministic display order is conventional JPEG/PNG reference → JXL/AVIF/WebP compressed candidate. External JPEG XL peers are labeled `external_rendered_peer` with unknown lineage; they are never presented as proven derivatives. Same-stem matching requires equal dimensions, compatible capture times when available, and all three conservative visual checks: 32×32 normalized RGB RMSE ≤ 0.08, dHash distance ≤ 10, and 16-bin RGB histogram L1 distance ≤ 0.12. Ambiguous or contradictory manual-state candidates remain separate and are recorded as conflicts.

Schema migration 24 adds compression profiles, ordered file-management rulesets and rules, presets, workspace active-ruleset state, and display-preview provenance. Migration 25 adds persistent File Management execution and operation snapshots; migration 26 adds explicit conflict policies and reviewed-target snapshots. The deterministic dry-run summary separates candidate and executable operations; primary storage deltas, estimated net space freed, and `temporary_space_upper_bound_bytes` include only executable operations. The browser must prepare an execution with the analyzed plan digest; the backend re-runs the planner, persists the complete immutable snapshot, and only then permits explicit confirmation and execution.

The application image boundary is `media/image_decode.py`; Pillow-supported images and JPEG XL through the installed `imagecodecs`/libjxl runtime use the same loader. JXL metadata, thumbnail, and display-preview work shares one prepared decode within a media pass, current previews are reused by fingerprint/provenance, LAR-IQA and OpenCLIP preparation use the canonical loader, and malformed files fail in isolation without retrying healthy batch members. JXL source files remain canonical for the enabled keep-source production path; app-owned display JPEGs may be created under `.archive-index/`, and managed production JXL derivatives are written only to reviewed workspace targets. RAW inspection uses a separate read-only `rawpy` sensor-development endpoint with `use_camera_wb`, `no_auto_bright`, sRGB, 8-bit display output, native stored orientation, and normal rawpy display gamma. The UI accepts -5..+5 EV in 0.5 steps and 5-unit coarse adjustments for the other controls; rawpy `exp_shift` is clamped to its supported -2..+3 EV range and the residual is applied through the documented `bright` stage. Requests are debounced, latest-request-wins, and cached in a bounded 16-entry/64 MiB memory LRU. Offline representations short-circuit the viewer before any preview request, and no RAW source is modified. Phase 10C provides validated JPEG XL keep-source execution; source replacement remains blocked by the metadata contract, while AV1 production and AVIF archival execution remain blocked. OCR, faces, and semantic clustering remain deferred.

Phase 10A second-pass presentation details: File Management keeps its structured Rules/Compression profiles/Plan UI free of raw JSON. Disk capacity is displayed in IEC units as current free space, with estimated post-plan free space when available; copy/move rules say `Recreate source folders inside destination`. Zero storage-delta lines are omitted. The comparison and RAW overlays reuse the shared camera lifecycle, and RAW eye actions never enter comparison.

The File Management dialog shows the supported mutation scope: Copy, Compress, Move, and Delete. Unsupported compression rows remain visibly blocked with codec-specific reasons.

The final targeted Phase 10A correction pass removes the visible rule enabled state. Rules read as `For [type] representations of [state] assets → [action]`, with representation type first; legacy disabled database rows remain omitted from the editable list. The RAW viewer uses normal display-referred gamma and native rawpy orientation. Its -5..+5 EV control clamps rawpy `exp_shift` to 0.25..8 and applies the remaining exposure with `bright`; the source remains read-only. Offline representation eyes open a message-only overlay without requesting an image. Comparison status distinguishes SHA-256 byte-identical duplicates from MSE-zero pixel-identical pairs, uses `Same size` instead of `1× smaller`, and presents readable `Reference → Compared` metrics. The global job line is `Indexing · MM:SS / MM:SS` or `Re-indexing · MM:SS / MM:SS`.

The last Phase 10A presentation/planner correction adds a packaged compression-preview sample at `web/assets/compression-preview/reference.jpg` with the original 2832×4240 dimensions. The three built-in JXL previews are precomputed by `scripts/generate_compression_previews.py`; their browser artifacts are lossless WebP files and `manifest.json` records the actual JXL byte sizes and decoded-pixel MSE. Built-in preview opening reads only these packaged assets and never encodes/decodes JXL or accesses workspace media. Custom JXL/AVIF previews use the same bundled reference, cache app-owned results under `.archive-index/compression-previews`, and key them by reference hash, codec, and complete settings. The preview dialog reuses the representation comparison camera, with Slider as the default and Side by side as the alternate; AV1 profiles do not expose image previews.

Copy, Move, and Compress rules use explicit left-aligned option wrappers. Their helper updates during destination input without replacing the input. Built-in selected RAW cleanup moves flat to `raws`; selected JPEG/PNG cleanup copies flat to `jpgs`; both default to Rename conflicts and do not recreate source folders. New rules persist `conflict_policy`; legacy `rename_on_conflict` values are read only as compatibility input. Dry-run target resolution uses case-insensitive Windows semantics and deterministic `file (1).ext`, `file (2).ext` suffixes across existing entries and earlier planned outputs. Source-replacing compression in place is represented as an intentional `replaces_source_in_place` operation; current JPEG XL source replacement is blocked because the installed `imagecodecs` encoder does not provide a safe standalone EXIF/ICC/XMP preservation path. Keep-source JPEG XL uses the same canonical quality/distance/effort settings as previews and records exact managed-derivative provenance. AV1 capability is probed once from the local FFmpeg `-version` and `-encoders` output. No archive source mutation is performed by previews or planning.
...
## Current checkpoint — Phase 10C

Phase 10C is complete for production JPEG XL Keep-source execution on supported JPEG and PNG sources without embedded ICC profiles. The application has the indexing/catalog foundation, quality, conservative grouping and review, reconciliation, RAW/JPEG representation handling, video quality, semantic search and Similar, modular frontend, Geo Map/Timeline/Vector Cloud, JXL decoding and comparison/Difference mode, RAW inspection, compression profiles/previews, File Management rulesets/presets, deterministic plan digests, persistent execution snapshots, source-fingerprint revalidation, execution-time containment and Windows-name validation, no-overwrite finalization, cancel/resume/retry, restart recovery, and post-mutation scan/managed-lineage/reconciliation. JPEG XL source replacement remains blocked until a metadata-preserving encoder path is available; ICC-tagged sources are blocked because the installed encoder cannot safely embed their profile; AV1 remains benchmarked/planning-only; AVIF remains preview-only.

Current File Management analysis reuses per-run directory listings and target-status/hash results, indexes operation membership for summary calculation, and reports cancellable phases through a small in-process session API. A manual decision carries the cached browser catalog forward only when the browser revision advanced by exactly that single decision, avoiding a full catalog rebuild during fullscreen filtered navigation without masking concurrent catalog changes. Fullscreen image fit is constrained to the actual media pane, including Info-open layouts; fullscreen image interaction is limited to the visible intersection of the rendered image and that pane, so letterboxed blank space remains a close target even when zoomed. Representation comparison inserts known display resources immediately, uses app-owned JXL previews where available, and computes cheap physical-pair metrics asynchronously. Difference is a separate lazy binary request using a vectorized log-scaled black-to-red map and fast PNG `compress_level=1`; Slider and Side-by-side never generate it. Comparison and Difference results use bounded workspace-scoped caches keyed by both physical identities, source/preview fingerprints, dimensions, and algorithm version; stale requests are ignored or cancelled, and image failures become explicit errors. The UI reports rounded global `MSE N`; max-pixel MSE remains Difference-internal and feeds the inverse-log colorbar labels. Difference validates decoded dimensions rather than trusting stale indexed dimensions. RAW inspection is read-only, uses a bounded 16-entry/64 MiB workspace-scoped memory cache keyed by source stat and all five coarse control values, and debounces latest-request-wins previews. RAW camera fit is initialized only after a preview has natural dimensions and layout has settled, so zoom 1 must show the complete centered image inside the actual RAW stage. A cache hit returns the already encoded PNG; each unique setting tuple currently performs its own rawpy decode/postprocess and display encoding. RAW controls use 0.5 EV and 5-unit coarse steps, with per-control and all-control resets.

## Phase 10B — safe File Management executor

Migration 27 adds `managed_derivative` to the migration 25/26 execution tables. A run stores the plan digest, frozen summary/metadata, actual effects, refresh status, and cancellation state; each operation stores its deterministic position, phase, operation type, source identity/path/size/mtime/SHA-256, target, conflict policy, reviewed-target size/mtime/SHA/type snapshot, rule/profile snapshot, conflicts/blockers, stage, temp path, output hash/size, progress, attempts, and error. Supported execution is single-threaded in `Copy → Compress → Move → Delete` order. Supported production compression is JPEG XL keep-source for JPEG/PNG; blocked Compress rows remain persisted as excluded snapshots with their capability reason.

Preparation accepts only the analyzed plan digest and ruleset identity; the backend re-runs the planner and rejects a changed digest. The review surface displays the frozen Copy/Move/Delete counts, blocked Compress rows, storage estimates, exclusions, and destructive acknowledgement before start. Every source and target is revalidated at the operation boundary, including regular-file checks, size/mtime/SHA-256 source verification, workspace containment, `.archive-index` exclusion, symlink/reparse rejection, and Windows-invalid segment rejection. Copy and cross-device Move stage to a persisted app-owned target-directory temp, hash and flush the output, and use no-overwrite hard-link finalization; same-filesystem Move uses the same no-overwrite primitive and falls back to copy/validate/finalize/delete when needed. Delete only unlinks the exact confirmed regular file after a fresh hash check and never removes directories or sibling representations.

The worker is sequential and process-coordinated so one workspace cannot have two mutation workers. A draft freezes a reviewed plan but does not lock settings or count as active mutation; the same ruleset/digest reuses it, a changed review supersedes older drafts, and the user can discard a draft. Only `running` and `cancelling` lock settings and indexing; an interrupted run blocks new execution until the user resumes or abandons it. Indexing, re-indexing, and configuration mutation are rejected while execution is active; execution is rejected while a pending/running workspace job exists. Cancellation stops at chunk boundaries, removes only the persisted owned temp, and preserves completed operations. Resume resets only cancelled/interrupted work; Retry failed resets only explicitly failed operations. Runs left running/cancelling after process loss become interrupted on workspace reopen. No byte-level resume is attempted; incomplete file work restarts from a safe boundary. After any filesystem change, the worker runs the existing scan and reconciliation path only; refresh failure is recorded separately and does not undo filesystem success.

Copy, Move, and Compress rules use explicit `Rename`, `Skip`, or `Overwrite` conflict policies, defaulting to Rename. Rename targets are frozen deterministically; Skip creates a persisted skipped row without writing; Overwrite is allowed only for an exact reviewed regular-file target snapshot and uses authorized replacement, never a newly appearing target. JPEG XL executes as source revalidation → target-directory owned temp → decode/encode → flush/fsync → decode/dimension/channel/catastrophic validation → exact no-overwrite finalization → provenance record → catalog refresh. Output that is not smaller than its source is skipped without writing a final derivative. The source remains untouched for the enabled keep-source path. `managed_derivative` records source/output paths and hashes, logical/source IDs, profile/settings, algorithm/version, encoder version, source disposition, and metadata contract; scan/reconciliation links the discovered output to the original logical asset and the UI labels it `JPEG XL derivative · <profile>`. Recovery recognizes a validated finalized output by its frozen output hash; interrupted encode work restarts the current image from zero. Cancellation takes effect before/after the blocking codec call and between sequential operations. Large-copy progress writes are throttled to time/byte intervals while final and cancellation updates remain persisted. Synthetic Phase 10B/10C mutation tests use disposable workspaces only; no real archive is used.

The explicit deletion invariant is: indexing, browsing, and maintenance never delete source media; cleanup is not automatic for rejected or redundant files; an exact source may be deleted only by an explicitly reviewed and confirmed File Management Delete operation after execution-time validation. JPEG XL source replacement remains blocked by the metadata contract; if enabled in a future phase, deletion must happen only after validated finalization and a second source fingerprint check. Future AV1 planning profiles are `AV1 1080p60 Fast`, `AV1 1080p60 Very Fast`, `AV1 4K120 Fast`, and `AV1 4K120 Very Fast`; max-resolution/FPS caps never upscale or increase FPS, the current default candidate is `AV1 4K120 Very Fast`, and the profile contract uses SVT-AV1 preset 8/10 as the benchmarked CPU candidate. AV1 execution remains blocked because representative archival quality, HDR/10-bit, VFR/timestamp, multi-stream preservation, packaging, and recovery validation are incomplete. AVIF remains preview-only.

## Phase 10C — production compression

JPEG XL is the primary image target. Production Keep-source execution currently supports JPEG and PNG sources without embedded ICC profiles, using the existing High Quality, Balanced, and High Compression profiles: quality 80/60/40, canonical `quality_to_distance`, and effort 7. The shared `media/jpegxl.py` contract is used by previews and production; the installed `imagecodecs`/libjxl 0.12.0 encoder is versioned in each frozen operation snapshot. ICC-tagged sources are planner-blocked because the installed `jpegxl_encode` API exposes no safe ICC/container-metadata preservation path. EXIF, GPS, and XMP are recorded as a source-retained metadata contract, not falsely claimed to be embedded in standalone JXL output.

The enabled lifecycle is source fingerprint revalidation → safe target-directory creation → app-owned persisted temp → decode/encode → flush/fsync → output SHA-256, decode, dimensions, exact alpha samples where present, and conservative catastrophic-visual checks → execution-time target revalidation → atomic no-overwrite finalization → managed-derivative provenance → scan → exact managed-lineage linking → reconciliation. A non-smaller output is recorded as `Skipped — compressed output was not smaller than the source.` Keep-source is the only enabled JXL disposition. Source replacement is planner-blocked because the current `imagecodecs.jpegxl_encode` API does not provide a safe standalone EXIF/ICC/XMP metadata preservation path; the source remains available and independently canonical for the enabled mode. Recovery restores the frozen metadata contract and adds `recovered: true`; no metadata-preservation claim may be added without a validated encoder/runtime contract.

Migration 27 adds `managed_derivative`, and migration 28 persists the frozen compression metadata contract on execution operations. Managed records store execution operation, source/output paths and SHA-256 values, source physical/logical IDs, profile/settings, algorithm/version, encoder version, source disposition, metadata contract, and the linked output physical file. A current link requires both exact output path and output SHA-256; historical records remain auditable, but only the newest exact record is materialized on one physical file. Scan refresh applies managed lineage before reconciliation, removes empty provisional logical assets, and excludes managed JXL from external same-stem heuristics. Browser queries defensively select one current managed row. External JXL remains unknown lineage. Conflict Rename/Skip/Overwrite semantics, frozen target snapshots, cancellation, retry, interruption recovery, no-byte-level resume, and catalog-refresh failure handling follow the Phase 10B executor.

The future AV1 built-ins are `AV1 1080p60 Fast`, `AV1 1080p60 Very Fast`, `AV1 4K120 Fast`, and `AV1 4K120 Very Fast`; the default candidate is 4K120 Very Fast. They are maximum resolution/FPS caps and never upscale or increase frame rate. `scripts/benchmark_av1.py` benchmarks disposable fixtures and records exact commands, encoder settings, output probes, PSNR/SSIM when available, size, speed, and stream-contract notes. The current FFmpeg 6.1.1 runtime exposes `libsvtav1`, `libaom-av1`, `librav1e`, and hardware candidates; the synthetic benchmark confirms working SVT-AV1 and rav1e CPU paths, with SVT preset 8/10 as the current Fast/Very Fast candidate. AV1 production remains blocked because representative archival quality, HDR/10-bit/color, VFR/timestamps, multi-stream preservation, packaging, and persistent recovery validation are incomplete. AVIF remains preview-only.
...
Long-term visualization roadmap: the planar Geo Map should eventually become horizontally world-wrapping and antimeridian-aware, with continuous longitude wrapping, repeated world cells, cross-±180° panning, and clustering/fitting that treats +179° and -179° as neighbors. Later evaluate an offline rotatable 3D globe that reuses indexed coordinates and bundled lightweight Natural Earth geometry with an orthographic/simple projection, without requiring online tiles or a GPU, while preserving thumbnail clustering, LOD, and hit testing. Neither feature is part of the current implementation.
...
- indexing, browsing, and maintenance never delete source media automatically; an exact source file may be deleted only by an explicitly reviewed and confirmed File Management Delete operation;
...
Historical derivative lineage can initially remain manual/unknown rather than risking false associations. Current RAW/JPEG pairing and logical-representation rules are implemented and are not deferred architecture questions.
...
- future Broad grouping criteria and behavior; current Strict grouping thresholds and construction rules are frozen.
...
6. Never implement automatic source deletion. Explicit File Management Delete is permitted only from a frozen, reviewed, confirmed plan after execution-time source validation; compression replacement deletion remains blocked until a validated standalone metadata-preserving encoder contract exists.
