...
Schema migration 24 adds compression profiles, ordered file-management rulesets and rules, presets, workspace active-ruleset state, and display-preview provenance. The deterministic dry-run summary includes Delete/Copy/Move/Compress counts and bytes, estimated net space freed, peak temporary space, surviving-representation warnings, conflicts, and executor availability. No file executor exists in Phase 10A: dry runs never change source media or filesystem paths.
