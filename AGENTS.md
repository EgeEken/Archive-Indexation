...
Geo Map uses valid GPS from the preferred active physical representation, then active fallbacks including RAW metadata. Latitude and longitude must be finite and within their normal geographic ranges. It has no reverse geocoding, location names, map tiles, CDN scripts, or runtime map-network calls. The basemap is a 251,749-byte app-owned GeoJSON reduction of Natural Earth 1:110m Admin 0 country boundaries (public domain; source: https://github.com/nvkelso/natural-earth-vector). Markers use Web-Mercator world coordinates and level-of-detail cells: `lod = floor(log2(scale / baseScale) * 1.5)`, clamped to 0–22, with `cellWorld = 1 / (64 * 2^lod)` and keys from `floor(worldX / cellWorld), floor(worldY / cellWorld)`. Each cell is drawn at its screen centroid. Count badges zoom toward compact cells; at high zoom a same-coordinate cell opens a bounded local asset strip. Representative thumbnails use highest finite quality with asset-ID tie-breaking.

Timeline has Capture time and File created time modes. Capture mode positions assets using deterministic UTC numeric coordinates built from the stored displayed date/time components. It intentionally does not reinterpret an EXIF-local-unknown timestamp as UTC, and explicit offsets do not shift the visible position. File created mode uses the persisted file-created time of the preferred active physical representation and uses the same displayed-component coordinate rule. It uses sorted points with binary-search visible ranges, adaptive histogram buckets whose width derives from the visible span and canvas width, representative thumbnails at broad spans, a single detailed thumbnail band with no semantic vertical encoding, adaptive year/month/date/hour/minute/second ticks, and bounded wheel-centered zoom and horizontal pan. Each time mode keeps its own transform; filter or revision changes refit the current mode.
...
The opt-in synthetic projection benchmark covers 10k, 50k, and 100k 8-dimensional vectors. On the local test machine, `_fit_pca` measured approximately 0.025s, 0.093s, and 0.197s respectively with the 10,000-asset fit cap. No separate browser paint benchmark was recorded; all three renderers remain Canvas-based with O(N) refresh work, screen-grid Geo clustering, binary-search Timeline visibility, bounded representative-thumbnail loading, and a screen-cell Vector click index. Focused backend, projection, frontend-contract, and three-test Playwright Chromium coverage were added. Broad grouping, responsive/mobile overhaul, and Phase 10 remain deferred.
