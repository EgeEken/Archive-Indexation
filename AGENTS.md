...
- `app-visualizations.js`: local Geo Map, Timeline, Vector Cloud, Canvas interaction, and shared visualization selection.
...
Geo Map uses valid GPS from the preferred active physical representation, then active fallbacks including RAW metadata. Latitude and longitude must be finite and within their normal geographic ranges. It has no reverse geocoding, location names, map tiles, CDN scripts, or runtime map-network calls. The basemap is a 2,155-byte app-owned low-resolution simplified GeoJSON resource. Markers are clustered dynamically in 42px screen-space grid cells; cluster labels are asset counts, clicking a cluster zooms while it remains compact, and a high-zoom identical-coordinate cluster opens a bounded asset list.

Timeline positions assets using deterministic UTC numeric coordinates built from the stored displayed date/time components. It intentionally does not reinterpret an EXIF-local-unknown timestamp as UTC, and explicit offsets do not shift the visible position. It uses sorted points with binary-search visible ranges, adaptive histogram buckets at broad spans, deterministic non-semantic lanes at detailed spans, adaptive year/month/date/hour/minute/second ticks, and bounded wheel-centered zoom and horizontal pan.
...
The opt-in synthetic projection benchmark covers 10k, 50k, and 100k 8-dimensional vectors. On the local test machine, `_fit_pca` measured approximately 0.021s, 0.083s, and 0.168s respectively with the 10,000-asset fit cap. No separate browser paint benchmark was recorded; all three renderers remain Canvas-based with O(N) refresh work, screen-grid Geo clustering, binary-search Timeline visibility, and a screen-grid Vector click index. Focused backend, projection, frontend-contract, and three-test Playwright Chromium coverage were added. Broad grouping, responsive/mobile overhaul, and Phase 10 remain deferred.
