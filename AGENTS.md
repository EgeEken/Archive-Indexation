...
Phase 10A second pass is complete. File Management is a first-class workspace-header action with structured Rules, Compression profiles, and Plan sections. Built-in Archive cleanup and Keep selected only presets are immutable; edits become Custom Ruleset, while custom presets can be saved, renamed, and deleted. The normal UI contains no rules/settings JSON or raw plan output. Rules use semantic asset and representation selectors, the planner evaluates every matching rule, supports planned Delete, reports destination collisions, and remains planning-only.

Logical assets remain distinct from their physical files and persisted representation relationships. Details exposes compact eye/Explorer actions; the eye opens a representation inspection overlay with Open/Details, and compatible representations support on-demand side-by-side or slider comparison. Comparison previews and MSE/PSNR use the actual decoded physical pair, not the ordinary JPEG display preview. External JPEG XL peers are labeled `external_rendered_peer` with unknown lineage; they are never presented as proven derivatives. Same-stem matching requires equal dimensions, compatible capture times when available, and all three conservative visual checks: 32×32 normalized RGB RMSE ≤ 0.08, dHash distance ≤ 10, and 16-bin RGB histogram L1 distance ≤ 0.12. Ambiguous or contradictory manual-state candidates remain separate and are recorded as conflicts.
