...
Vector Cloud uses the same world camera and a cached approximately 512×512 world-anchored Gaussian density texture keyed by filtered data revision, with sqrt normalization and no semantic axis names or fake clusters. Stable x/y/LOD cells reveal larger representative thumbnails as the view zooms; image/video fallback colors remain restrained and selected assets use the normal viewer path. The opt-in synthetic projection benchmark covers 10k, 50k, and 100k 8-dimensional vectors. On the local test machine, `_fit_pca` measured 0.039s, 0.097s, and 0.191s respectively with the 10,000-asset fit cap. No separate browser paint benchmark was recorded; all three renderers remain Canvas-based with O(N) data refresh work and no per-asset DOM nodes. Groupings expose stable `Group N` labels for real multi-image groups. Focused backend, projection, frontend-contract, and three-test Playwright Chromium coverage were added. Broad grouping, responsive/mobile overhaul, and Phase 10 remain deferred.
