...
JPEG XL is the primary image target. Production execution currently supports JPEG and PNG sources with the existing High Quality, Balanced, and High Compression profiles: quality 80/60/40, canonical `quality_to_distance`, and effort 7. The shared `media/jpegxl.py` contract is used by previews and production; the installed `imagecodecs`/libjxl 0.12.0 encoder is versioned in each frozen operation snapshot.

The enabled lifecycle is source fingerprint revalidation → safe target-directory creation → app-owned persisted temp → decode/encode → flush/fsync → output SHA-256, decode, dimensions, channels/alpha, and conservative catastrophic-visual checks → execution-time target revalidation → atomic no-overwrite finalization → managed-derivative provenance → scan/reconciliation. A non-smaller output is recorded as `Skipped — compressed output was not smaller than the source.` Keep-source is the only enabled JXL disposition. Source replacement is planner-blocked because the current `imagecodecs.jpegxl_encode` API does not provide a safe standalone EXIF/ICC/XMP metadata preservation path; the source remains available and independently canonical for the enabled mode. No metadata-preservation claim may be added without a validated encoder/runtime contract.

Migration 27 adds `managed_derivative`, which records execution operation, source/output paths and SHA-256 values, source physical/logical IDs, profile/settings, algorithm/version, encoder version, source disposition, metadata contract, and the linked output physical file. Reconciliation links known managed outputs to the source logical asset and the Details UI labels them `JPEG XL derivative · <profile>`; filename similarity alone never creates managed lineage. External JXL remains unknown lineage. Conflict Rename/Skip/Overwrite semantics, frozen target snapshots, cancellation, retry, interruption recovery, no-byte-level resume, and catalog-refresh failure handling follow the Phase 10B executor.
...
6. Never implement automatic source deletion. Explicit File Management Delete is permitted only from a frozen, reviewed, confirmed plan after execution-time source validation; compression replacement deletion remains deferred until Phase 10C.
